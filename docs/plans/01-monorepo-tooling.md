...
- **Out-of-sequence addition (commit `1283376`, between Phase 01 and Phase 02, undated in the original log — landed 2026-07-25/26):** the public marketing landing page (`apps/web/src/app/page.tsx` + `apps/web/src/components/landing/**`, ~1,950 lines) was built ahead of its natural place in the plan (Phase 06 is where `apps/web` gets real UI). It sits at the root route `/`, separate from the future `(dashboard)` route group Phase 06 will add. Recorded here, against this phase, because this is where `apps/web`'s shell lives — no dedicated phase covers it.
  - Adds `lenis` (smooth-scroll) as a direct `apps/web` dependency pinned to an exact version (`1.1.13`) in `apps/web/package.json` — **not** routed through the root `catalog:` block that every other shared dependency uses (D5 precedent, `pnpm-workspace.yaml`). Deviation, not yet reconciled with the catalog convention; flagged rather than silently normalized, since only one package uses it so far.
  - No tests written for the landing components (pure presentational/motion code, no logic to unit-test yet).
